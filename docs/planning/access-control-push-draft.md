...
| Level | Alias | Capabilities |
|-------|-------|-------------|
| 0 | `none` | No access (blocked/revoked) |
| 1 | `read` | Pull assets shared with them |
| 2 | `read+write` | Pull + push new assets to your node |
| 3 | `read+write+modify` | Pull + push + overwrite/delete existing assets |

Permissions are checked at two layers:

```
Contact-level permission (global ceiling)
  └─ Folder/asset-level permission (scoped override, can only narrow or match)
```

A contact with global `read` can never write, even if a folder says `read+write`. A contact with global `read+write` can be narrowed to `read` on a specific folder.
...
1. ~~**Push approval mode?**~~ → **Resolved:** Auto-accept if authorized. Reject unknown/unauthorized silently — users sort it out themselves. No notification system needed.
...
4. **Quota/rate limiting on push?** — prevent a contact from filling your disk even with write access
5. **Group nesting?** — can a group contain other groups? (Suggest: no, keep flat for v1)
6. **Folder depth limit?** — infinite nesting or cap at N levels?
7. **Chat session UX** — separate UI surface, or just another folder you "open"?
